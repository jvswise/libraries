...
// You are free to use/extend this library but please abide with the CCSA
// license:
// http://creativecommons.org/licenses/by-sa/3.0/
// 2013-06-14 (C) felix@lowpowerlab.com
// ****************************************************************************
#include <RFM69_DMX.h>
#include <RFM69registers_DMX.h>
#include <SPI.h>
...
volatile byte RFM69::ACK_RECEIVED; // Should be polled immediately after
                                   //  sending a packet with ACK request
volatile int RFM69::RSSI; // Most accurate RSSI during reception (closest to
                          //  the reception)
...
bool RFM69::initialize(byte freqBand, byte nodeID, byte networkID)
{
  const byte CONFIG[][2] =
  {
    ...
    /* 0x25 */ { REG_DIOMAPPING1, RF_DIOMAPPING1_DIO0_01 }, // DIO0 is the only
                                                            //  IRQ we're using
    /* 0x29 */ { REG_RSSITHRESH, 220 }, // Must be set to dBm =
                                        //  (-Sensitivity / 2)
                                        // - default is 0xE4=228 so -114dBm
    /* 0x2d */ //{ REG_PREAMBLELSB, RF_PREAMBLESIZE_LSB_VALUE }
        // default 3 preamble bytes 0xAAAAAA
    /* 0x2e */ { REG_SYNCCONFIG, RF_SYNC_ON | RF_SYNC_FIFOFILL_AUTO |
                 RF_SYNC_SIZE_2 | RF_SYNC_TOL_0 },
    /* 0x2f */ { REG_SYNCVALUE1, 0x2D },  // Attempt to make this compatible
                                          // with sync1 byte of RFM12B lib
    ...
    /* 0x3C */ { REG_FIFOTHRESH, RF_FIFOTHRESH_TXSTART_FIFONOTEMPTY |
                 RF_FIFOTHRESH_VALUE }, //TX on FIFO not empty
    /* 0x3d */ { REG_PACKETCONFIG2, RF_PACKET2_RXRESTARTDELAY_2BITS |
                 RF_PACKET2_AUTORXRESTART_ON | RF_PACKET2_AES_OFF },
           // RXRESTARTDELAY must match transmitter PA ramp-down time
           // (bitrate dependent)
    /* 0x6F */ { REG_TESTDAGC, RF_DAGC_CONTINUOUS }, // Run DAGC continuously
                                                     //  in RX mode
    {255, 0}
  };
  ...
  SPI.setClockDivider(SPI_CLOCK_DIV2); // Max speed, except on Due which can
...
void RFM69::setMode(byte newMode)
{
  ...
  while (_mode == RF69_MODE_SLEEP && (readReg(REG_IRQFLAGS1) &
...
bool RFM69::send(byte toAddress, const void* buffer, byte bufferSize,
                 bool requestACK)
{
...
// To increase the chance of getting a packet across, call this function
// instead of send and it handles all the ACK requesting/retrying for you :)
// The only twist is that you have to manually listen to ACK requests on
// the other side and send back the ACKs.
// The reason for the semi-automaton is that the lib is interrupt driven and
// requires user action to read the received data and decide what to do with it.
// Replies usually take only 5 - 8ms at 50kbps @ 915Mhz
bool RFM69::sendWithRetry(byte toAddress, const void* buffer, byte bufferSize,
                          byte retries, byte retryWaitTime)
{
  long sentTime;
...
bool RFM69::sendACK(const void* buffer, byte bufferSize)
{
  byte sender = SENDERID;
  while (!canSend())
    receiveDone();
  return sendFrame(sender, buffer, bufferSize, false, true);
}


bool RFM69::sendFrame(byte toAddress, const void* buffer, byte bufferSize,
                      bool requestACK, bool sendACK)
{
  ...
  /* No need to wait for transmit mode to be ready since its handled by
   * the radio
   */
...
void RFM69::interruptHandler() {
  //pinMode(4, OUTPUT);
  //digitalWrite(4, 1);
  if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) &
      RF_IRQFLAGS2_PAYLOADREADY))
  {
    ...
    // Match this node's address, or broadcast addr 0x0 or anything in
    // promiscuous mode
    if (!(_promiscuousMode || TARGETID==_address || TARGETID==0))
    {
      PAYLOADLEN = 0;
      unselect();
      //digitalWrite(4, 0);
...
// OFF = Enable node+broadcast filtering to capture only frames sent to
//         this/broadcast address
void RFM69::promiscuous(bool onOff)
{
  _promiscuousMode=onOff;
  //writeReg(REG_PACKETCONFIG1, (readReg(REG_PACKETCONFIG1) & 0xF9) |
...
void RFM69::setHighPower(bool onOff)
{
  ...
  if (_isRFM69HW) //turning ON
    ...
    writeReg(REG_PALEVEL, RF_PALEVEL_PA0_ON | RF_PALEVEL_PA1_OFF |
                          RF_PALEVEL_PA2_OFF | _powerLevel); // Enable P0 only
}
